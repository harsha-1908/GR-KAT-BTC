...
# load a yaml file containing key-values that override the defaults for
# the main parser below
...
def main():
    ...
    if args.pretrained_path:
        # merge with pretrained_cfg of model, 'file' has priority over 'url'
        # and 'hf_hub'.
    ...
    if args.distributed and args.sync_bn:
        ...
        if has_apex and use_amp == "apex":
            ...
            # WARNING this won't currently work with models using
            # BatchNormAct2d
    ...
    if use_amp == "apex":
        ...
        if device.type == "cuda" and amp_dtype == torch.float16:
            # loss scaler only used for float16 (half) dtype, bfloat16 does not
            # need it
    ...
    # setup exponential moving average of model weights, SWA could be used
    # here too
    model_ema = None
    if args.model_ema:
    ...
    if mixup_active:
        ...
        if args.prefetcher:
            # collate conflict (need to support de-interleaving in collate
            # mixup)
    ...
    if args.val_split:
        ...
        if args.distributed and ("tfds" in args.dataset or "wds" in args.dataset):
            # FIXME reduces validation padding issues when using TFDS, WDS w/
    ...
    if args.jsd_loss:
        ...
        # smoothing is handled with mixup target transform which outputs
        # sparse, soft targets
        if args.bce_loss:
    ...
    if utils.is_primary(args) and args.log_wandb:
        if has_wandb:
            wandb.init(project="scale-kan", name=exp_name, config=args)
        else:
            _logger.warning(
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if output_dir is not None:
                if eval_metrics is not None:
                    eval_metrics.pop("preds", None)
                    eval_metrics.pop("targets", None)
...
def validate(
    ...
    # --- Save preds/targets as .npz for later analysis ---
    if utils.is_primary(args) and epoch is not None:
        preds_targets_dir = os.path.join(
            args.output if args.output else ".", "preds_targets"
        )
        os.makedirs(preds_targets_dir, exist_ok=True)
        save_path = os.path.join(preds_targets_dir, f"epoch_{epoch}{log_suffix}.npz")
        np.savez_compressed(
            save_path, preds=all_preds.numpy(), targets=all_targets.numpy()
        )
        _logger.info(f"Saved preds/targets to {save_path}")
    ...
    if all_preds.numel() > 0 and all_targets.numel() > 0:
        ...
        save_path = os.path.join(
            args.output if args.output else ".", f"confusion_matrix{log_suffix}.png"
        )
        plt.savefig(save_path, dpi=150)
        plt.close()

        if utils.is_primary(args):
            _logger.info(f"Saved confusion matrix to {save_path}")
